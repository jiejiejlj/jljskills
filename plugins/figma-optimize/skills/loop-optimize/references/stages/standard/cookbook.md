...
`standard` 评审设计规范板本身时用到的 **skill 专属** Figma Plugin API 配方。所有 `use_figma` 调用**前必先走 `figma-use` skill**。

> 共享判据与通用配方——库变量误判坑、三源 API、`setBoundVariableForPaint` 重赋值、切页、纪律四条——**唯一权威在 [`../../figma-facts/SKILL.md`](../../figma-facts/SKILL.md)**(flow P1 已装载),本文件不复述。
