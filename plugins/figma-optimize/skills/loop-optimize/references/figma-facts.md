...
figma-optimize 的单一真相源: 各阶段共用的 Figma Plugin API 判据与写回纪律. 各 stage flow 在 P1 以硬性步骤 Read 本文件装载; 判据正文只存这里, 各 flow 只留指针, 不复述. 阶段专属配方 (字体归正, 收敛 token, 组件化写回等) 仍在各自的 `references/figma-api-cookbook.md`.
