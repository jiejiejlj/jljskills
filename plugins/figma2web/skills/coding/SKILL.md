---
name: coding
description: 全程离线读一份 section spec + 本地切图 + 设计标准,复现出贴合技术栈的 web 代码与 Docker 产物,禁止魔法值。不调 figma-mcp。仅当用户主动用 `/figma2web:coding` 指令调用时使用 —— 不要在普通对话里自行触发。
...
## 用途
`page2doc` 之后,**离线**读 spec 出代码。输入:目标 section spec + 本地切图 + `tokens.md` + `project.md`。产 `app/` 源码 + Docker 产物。
...
## 何时运行
仅当用户主动用 `/figma2web:coding` 指令调用时运行。前置:`project.md` + `tokens.md` 存在;目标 spec 存在且已 finalize;**spec G 段切图必须在磁盘上(缺切图 HARD STOP)**;superpowers 已装。
...
2. **出实现计划**:几何翻译 / token 翻译 / 响应式 / 代码落位。
...
> 完整分阶段流程、翻译规则、两层验收见 [references/flow.md](references/flow.md) —— **动笔前先读它**。
