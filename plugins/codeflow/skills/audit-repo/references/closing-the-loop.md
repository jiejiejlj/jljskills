...
Modifier on any planning invocation (`/codeflow:audit-repo --issues`, `/codeflow:audit-repo security --issues`). The flag is the user's authorization to create issues — never create them without it.
...
4. Per plan: `gh issue create --title "<plan title>" --body-file <plan file>`. Labels: `audit-repo` plus the category — apply only if the labels exist or can be created without erroring; skip labels rather than fail.
