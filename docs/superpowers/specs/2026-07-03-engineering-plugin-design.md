...
3. **术语语言**：八术语保留英文原词（module / interface / implementation / depth / seam / adapter / leverage / locality，以及 deep/shallow），定义与正文用简体中文。禁用词清单保留英文（component / service / unit / API / signature / boundary / layer / wrapper）。理由：英文原词是预训练好的引导词，禁用词翻译后防漂移机制失效。
...
- **八术语表**：每条 = 英文原词 + 中文定义 + `_禁用_` 行。含 Implementation vs Adapter 的正交区分（小 adapter 大实现 vs 大 adapter 小实现）。
...
- `allowed-tools` 按需收窄：design-rules 只需 Read；improve-arch 需 Read / Grep / Glob / Bash / Write / Edit / Agent / AskUserQuestion；grilling 需 Read / Grep / Glob / Agent / Write / Edit / AskUserQuestion；domain-modeling 需 Read / Grep / Glob / Edit / Write / AskUserQuestion。
...
实施后终审发现并修复：禁用词清单补全 layer/wrapper（计划誊写时自 spec 漏抄）；grilling/improve-arch 的 allowed-tools 补 Agent/Write/Edit 以覆盖自身路由出口；「七术语」计数订正为八；improve-arch 与 html-report/grilling 间两处重复剪除；Mermaid 示例 linkStyle 修正。以仓库实际文件为准。

## 修订：文档位置统一（2026-07-03）

本插件组在目标项目侧读写的文档统一收在 `docs/jljskills/engineering/` 下（逐项经用户确认）：

- `CONTEXT.md` 由项目根迁入该目录；多 context 时 `CONTEXT-MAP.md` 同目录 + `contexts/<名>.md` 集中收纳，废弃原「就近放 src/*/」的分散式。
- ADR 由 `docs/adr/` 迁入 `adr/`；多 context 的 ADR 也集中，文件名加 context 前缀。
- grilling 新增收尾落盘：interface 草图 + 分支结论写 `designs/<日期>-<module名>.md`。
- improve-arch 的 HTML 审查报告维持 OS 临时目录不入库（一次性呈现物，且 SRI 安全边界依赖本地一次性）。
